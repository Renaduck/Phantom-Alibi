...
import { GameState, SaveGames, Item } from './types';
import { playPageFlipSound, playSwooshSound, playBackgroundMusic, playItemAcquiredSound } from './utils/audio';
import { completeTypingAnimation as completeTypingAnimationUtil } from './utils/dialogue';
import { fetchStoryData } from './utils/scene';

interface EnvironmentState {
  inventory: string[];
  addItem: (name: string, item: Item) => void;
  clearItems: () => void;
  toggleCarousel: () => void;
  showCarousel: () => void;
  hideCarousel: () => void;
...
interface StoreState extends GameState, EnvironmentState, SaveLoadState, UIState, AudioState, SceneState, EventState {
  ...
  setTypingInterval: (interval: number | null) => void;
...
const useStore = create<StoreState>((set, get) => ({
  ...
  currentDialogueContent: undefined,
  ...
  saveGame: () => {
    ...
    const saveData = {
      ...
      preview: get().currentDialogueContent?.substring(0, 30) + '...' || 'New game'
    };
  ...
  completeTypingAnimation: () => {
    const { typingInterval, currentDialogueContent, currentDialogueElement } = get();
    
    if (typingInterval !== null) {
      completeTypingAnimationUtil(
        typingInterval, 
        currentDialogueContent, 
